...
std::vector<PNP_ADAPTER> get_pnp_adapters()
{
	...
	while (1)
	{
		...
		SetupDiGetDevicePropertyW(device_info, &data, &DEVPKEY_Device_ProblemCode, &data_type, (BYTE*)buffer, sizeof(buffer), &len, 0);

		unsigned long device_status = *((unsigned long*)buffer);
		...
		adapter.driver_status = device_status;
